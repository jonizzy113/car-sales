...
//  for each looping throuh all keys
salesByWeek.forEach(sale => {
    const div = document.createElement("div");
    const head = document.createElement("h1");
    // getting the sales name and appending to div
        head.textContent = `${sale.sales_agent.first_name} ${sale.sales_agent.last_name}`
        div.appendChild(head)
        // getting keys of vehicle and appending
        for(const vehicle of Object.entries(sale.vehicle)) {
            const vehicleObject = document.createElement("div");
            vehicleObject.textContent = `${vehicle[0]}: ${vehicle[1]}`
            div.appendChild(vehicleObject)
        }
        // getting gross profit and appending
        const babyhead = document.createElement("h3")
        babyhead.textContent =  `Profit: $${sale.gross_profit}`
        div.appendChild(babyhead);
